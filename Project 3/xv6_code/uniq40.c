		...
		for(int p = 0; p < offset + 1; p++){
			for(int q = p + 1; q < offset; q++){
                ...
                if(comparison_res == 0){
					...
					if(p == 0){
						// Do nothing. Prevents segmentation error.
					}
					else if(casecmp(bufArray[p-1], bufArray[p]) == 0){ // Check to see if there is an identical value behind this one. If so don't print it. (We only want to print the first of a series)
						if(iFlag == 1){
							skipCondition = 1;
						}
						else if(strcmp(bufArray[p-1], bufArray[p]) == 0){
							skipCondition = 1;
						}
					}
					countArray[q]++;
				...
				else if(countArray[p] > 1 && p > 0){ // Check to see if it's the last of a series of duplicates. Can be fragmented. Need one outside the iner loop as well in the event there is not another duplicate in the array ahead of this one.
					if (iFlag == 1) { // Due to the nature of our program it only knows when there is a duplicate somewhere in front so this will ensure we don't print anything extra when dealing with the last of a series.
