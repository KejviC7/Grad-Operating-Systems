...
trap(struct trapframe *tf)
{
  ...
  default:
    ...
    // NEW CODE
    // We need to respond to PAGE FAULT from user space (tf->cs&3 == 3). In traps.h T_PGFLT = 14
    else if (tf->trapno == 14 && (tf->cs&3) == 3){
        ...
        faulty_addr = rcr2();
		cprintf("The fault address: %d\n", faulty_addr);

        // Round down virtual page to get boundary
        bound = PGROUNDDOWN(faulty_addr);
		cprintf("The boundary: %d\n", bound);
        ...
        // Mapping 
        if(mappages(myproc()->pgdir, (char*)bound, PGSIZE, V2P(mem), PTE_W|PTE_U) < 0){
